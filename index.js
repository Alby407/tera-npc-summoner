module.exports = function SalchySummonerMaster(script) {
	...
	for (let i = 0; i < summons.length - 1; i++) {
		script.command.add(summons[i].name, () => {
			let buffer = Buffer.alloc(4);
			buffer.writeUInt32LE(Number(summons[i].value));
			script.send('C_REQUEST_CONTRACT', 50, {
				ContractType: summons[i].type,
				NpcCreatureId: summons[i].npcId,
				ValueParam: Number(summons[i].value),
				ContractRequestee: "",
				Param: buffer
			});
		});
